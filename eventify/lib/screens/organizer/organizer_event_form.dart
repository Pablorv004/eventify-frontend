import 'package:eventify/config/app_colors.dart'; // Add this import
...
class _OrganizerEventFormState extends State<OrganizerEventForm> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(),
      body: FutureBuilder(
        future: eventProvider.fetchCategories(),
                    ...
                    Center(
                      child: Text(
                        ...
                        style: const TextStyle(
                            fontSize: 30, color: AppColors.deepOrange),
                      ),
                    ),
                    const Divider(color: AppColors.vibrantOrange, thickness: 2),
                    Expanded(
                      child: ListView(
                        children: [
                          TextFormField(
                          ...
                          DropdownButtonFormField<String>(
                            ...
                            items: eventProvider.categoryList
                                .toSet()
                                .map((category) {
                          ...
                          ListTile(
                            title: Text(
                                'Start Time: ${_startTime.toLocal().toString().split(' ')[0]} ${_startTime.toLocal().toString().split(' ')[1].substring(0, 5)}'),
                            trailing: const Icon(Icons.calendar_today),
                            onTap: () => _selectDateTime(context, true),
                          ),
                          ListTile(
                            title: Text(
                                'End Time: ${_endTime.toLocal().toString().split(' ')[0]} ${_endTime.toLocal().toString().split(' ')[1].substring(0, 5)}'),
                            trailing: const Icon(Icons.calendar_today),
                            onTap: () => _selectDateTime(context, false),
                          ),
                          const Divider(
                              color: AppColors.vibrantOrange, thickness: 2),
                            ElevatedButton(
                            style: ElevatedButton.styleFrom(
                              foregroundColor: Colors.white, backgroundColor: AppColors.vibrantOrange, // Text color
                            ),
                            onPressed: () async {
                              ...
                              if (_startTime.isBefore(DateTime.now())) {
                                ...
                                const SnackBar(
                                  content: Text(
                                    'Start time must be after today')),
                                );
                                return;
                              }
                              ...
                              final event = Event(
                                ...
                                latitude: widget.event != null
                                  ? double.tryParse(
                                    _latitudeController.text)
                                  : null,
                                longitude: widget.event != null
                                  ? double.tryParse(
                                    _longitudeController.text)
                                  : null,
                                maxAttendees: widget.event != null
                                  ? int.tryParse(
                              ...
                              await eventProvider.fetchEventsByOrganizer(
